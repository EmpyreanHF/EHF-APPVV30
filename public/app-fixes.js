document.addEventListener('DOMContentLoaded', function() {
            ...
            function createCrisisPostOnFeed(crisisData) {
                ...
                // CRITICAL: Use the ID from crisisData, do NOT generate a new one.
                // Generating a new timestamp breaks the duplicate guard in the listener.
                postElement.dataset.postId = crisisData.id || `crisis-${Date.now()}`;
                ...
                postElement.innerHTML = `
                    <div class="story-header">
                        ...
                        <div class="story-user-info"><strong>Crisis Report: ${crisisData.type}</strong><span>Reported by ${crisisData.username} · ${crisisData.createdAt ? new Date(crisisData.createdAt).toLocaleString('en-GB',{day:'numeric',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}) : 'Recently'}</span></div>
                    ...
                    switch (form.id) {
                        ...
                        case 'create-post-form': {
                            ...
                            (async () => {
                                try {
                                    ...
                                    // ── profile-dash-feed rendering REMOVED ─────────────────────────
                                    // The posts listener (line 2410-2428) already mirrors own posts
                                    // to profile-dash-feed. Optimistic local injection here caused
                                    // duplicate posts: one as "Guest" (optimistic, wrong author data),
                                    // one as the real username (from Firestore listener).
                        ...
                        case 'live-comment-form': {
                         ...
                         case 'help-form': {
                            ...
                            (async () => {
                                try {
                                    ...
                                    if (typeof window.uploadToCloudinary !== 'function') {
                                        showNotification('Upload service not ready. Please try again.', 'error');
                                        if (sosSubmitBtn) sosSubmitBtn.disabled = false;
                                        ...
                                        for (const file of sosMediaFiles) {
                                            try {
                                                const url = await window.uploadToCloudinary(file, (pct) => {
                                                    ...
                                                    const progressBar = document.querySelector('#sos-upload-progress .upload-progress-bar');
                         ...
                         case 'crisis-form': {
                            ...
                            let crisisProgressBar = document.getElementById('crisis-upload-progress');
                            if (!crisisProgressBar) {
                                crisisProgressBar = document.createElement('div');
                                crisisProgressBar.id = 'crisis-upload-progress';
                                crisisProgressBar.innerHTML = '<div class="upload-progress-container"><div class="upload-progress-bar" style="width:0%"></div></div>';
                                form.insertBefore(crisisProgressBar, crisisSubmitBtn);
                            }
                            crisisProgressBar.style.display = 'block';
                            ...
                            (async () => {
                                try {
                                    ...
                                    if (typeof window.uploadToCloudinary !== 'function') {
                                        showNotification('Upload service not ready. Please try again.', 'error');
                                        if (crisisSubmitBtn) crisisSubmitBtn.disabled = false;
                                        const cpb = document.getElementById('crisis-upload-progress');
                                    ...
                                    const mediaUrls = [];
                                    let uploadedCount = 0;
                                    const totalFiles = crisisMediaFiles.length;
                                    
                                    for (const file of crisisMediaFiles) {
                                        try {
                                            const url = await window.uploadToCloudinary(file, (pct) => {
                                                const overallProgress = Math.floor(((uploadedCount / totalFiles) + (pct / 100 / totalFiles)) * 100);
                                                showNotification(`Uploading crisis media: ${overallProgress}%`, 'info');
                                                const progressBar = document.querySelector('#crisis-upload-progress .upload-progress-bar');
                                                if (progressBar) progressBar.style.width = overallProgress + '%';
                                            });
                                            mediaUrls.push({ url, type: file.type });
                                            uploadedCount++;
                                        } catch(e) { 
                                            console.warn('Crisis media upload failed:', e);
                                            mediaUrls.push({ url: URL.createObjectURL(file), type: file.type }); 
                                            uploadedCount++;
                                        }
                                    }
                                    ...
                                    // ── DOM rendering omitted — onSnapshot listener renders it ──
                                    // The _crisisListener is the SINGLE source of truth across all
                                    // devices. Optimistic local injection caused duplicate posts.
            ...
            (function() {
                ...
                (function buildMobileBottomNav() {
                    ...
                    const userItems = [
                        ...
                        { t: 'news',           icon: 'fa-newspaper',        label: 'News'     },
                        { t: 'marketplace',    icon: 'fa-store',            label: 'Market'   },
                        { t: 'crisis-alert',   icon: 'fa-exclamation-triangle', label: 'Crisis' },
                ...
                (function() {
                    if (window.isGuest || !userState.id) return; // Only for logged-in users
                    if (document.getElementById('quick-post-fab')) return; // Already exists
                    
                    // Create floating action button (FAB)
                    const fab = document.createElement('button');
                    fab.id = 'quick-post-fab';
                    fab.innerHTML = '<i class="fas fa-pen"></i>';
                    fab.title = 'Quick Post';
                    fab.style.cssText = `
                        position: fixed;
                        bottom: 85px;
                        right: 20px;
                        ...
                        color: white;
                        border: none;
                        box-shadow: 0 4px 12px rgba(27,43,139,0.3);
                        cursor: pointer;
                        font-size: 20px;
                        z-index: 999;
                    ...
                    fab.addEventListener('mouseenter', function() {
                        ...
                        fab.style.boxShadow = '0 6px 20px rgba(27,43,139,0.4)';
                    });
                    fab.addEventListener('mouseleave', function() {
                        fab.style.transform = 'scale(1)';
                        fab.style.boxShadow = '0 4px 12px rgba(27,43,139,0.3)';
                    });
                        ...
                        ">
                            ...
                            <div class="quick-post-body" style="padding: 20px;">
                                <div class="quick-post-author" style="
                                    ...
                                    margin-bottom: 16px;
                                ">
                                    <img src="${userState.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(userState.fullName || 'User') + '&background=5B0EA6&color=fff&size=150'}" 
                                        style="
                                            width: 48px;
                                            height: 48px;
                                            border-radius: 50%;
                                        ...
                                        ">
                                    <div>
                                        <strong style="color: var(--primary); font-size: 0.95rem;">${userState.fullName || userState.username}</strong>
                    ...
                    fab.addEventListener('click', function() {
                        ...
                        document.getElementById('quick-post-text').focus();
                    });
                    
                    // Close modal handlers
                    modal.querySelector('.quick-post-close').addEventListener('click', function() {
                        ...
                        quickPostMediaFiles.forEach(function(file, index) {
                            ...
                            if (isVideo) {
                                wrapper.innerHTML = `
                                    ...
                                    <div style="position: absolute; top: 4px; left: 4px; background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem;">
